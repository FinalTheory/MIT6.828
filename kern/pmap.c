...
// This function should return virtual address as used in kernel code
// in kernel.ld, we link the kernel into "0xF0100000"
static void *
boot_alloc(uint32_t n)
{
	...
	// 'end' is a magic symbol automatically generated by the linker, "PROVIDE(end = .);"
	...
	n = ROUNDUP(n, PGSIZE);
	if (nextfree + n < nextfree) { return NULL; }
	result = nextfree;
	nextfree += n;
	return result;
}
...
page_alloc(int alloc_flags)
{
	if (page_free_list != NULL) {
		...
		alloc->pp_link = NULL;
		assert(alloc->pp_ref == 0);
		if (alloc_flags & ALLOC_ZERO) { memset(page2kva(alloc), 0, PGSIZE); }
		return alloc;
	}
	return NULL;
}
...
{
	if (pp->pp_ref) { panic("pp_ref is nonzero"); }
	if (pp->pp_link) { panic("pp_link is not NULL"); }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	pde_t pde = pgdir[pdx];
	if (pde & PTE_P) {
		physaddr_t pta = PTE_ADDR(pde);
		pte_t *pgtbl = KADDR(pta);
		return &pgtbl[ptx];
	} else {
		if (create) {
			// allocate new page fill zero
			struct PageInfo *page = page_alloc(ALLOC_ZERO);
			if (page) {
				// fill in the page dir
				physaddr_t pta = page2pa(page);
				pgdir[pdx] = pta | PTE_P | PTE_U | PTE_W;
				page->pp_ref++;
				pte_t *pgtbl = KADDR(pta);
				return &pgtbl[ptx];
			} else {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	const int mask = perm | PTE_P;
    for (int i = 0; i < len; i++) {
        void *addr = (void*)va + i;
        pte_t *pte_p = NULL;
        pde_t pde = env->env_pgdir[PDX(addr)];
        struct PageInfo *p = page_lookup(env->env_pgdir, addr, &pte_p);
        if (!p || !pte_p) {
            user_mem_check_addr = (uintptr_t)addr;
            return -E_FAULT;
        }
        pte_t pte = *pte_p;
        if (((pde & mask) != mask) || ((pte & mask) != mask)) {
            user_mem_check_addr = (uintptr_t)addr;
            return -E_FAULT;
        }
    }
